...
"crossplane-azure": {
  type:        "component"
  description: "Component for deploying an Azure Provider powered by EvoCloud/Crossplane."
...
template: {
  ...
  parameter: {
    //+usage=The name of the Azure ProviderConfig. Specify a unique name, defaults to `iaas-cp-azure-provider`
    provider_name: *"iaas-cp-azure-provider" | string

    //+usage=Run the following Azure CLI command to create a service principal and get the required values:
		// az ad sp create-for-rbac --sdk-auth --role Owner --scopes="/subscriptions/<your-subscription-id>" -n "evocloud-agent-sp" > "creds.json"
		// Use the `clientId`, `clientSecret`, `tenantId`, and `subscriptionId` from the `creds.json` file as parameters for the Azure ProviderConfig.
